...
public class ArraySequence<E> extends ArrayList<E> implements Sequence<E> {
    ...
    private class Index implements Position<E> {
        ...
        public E getElement() {
            return get(index); // using get from ArrayList<E>
        }
    ...
    public int indexAtPosition(Position<E> pos) throws IllegalArgumentException {
        if (!(pos instanceof ArraySequence.Index))
            throw new IllegalArgumentException("position is instance of ArraySequence.Index");
	...
	*/
    ...
    private E removeAtPos(int i) {
        ...
        for (int j = i; j < posList.size(); j++) {
            posList.get(j).setIndex(j);
        } // correct the index of cells that moved back
